  ...
  class UpdateCarrierSettingsRequestBody
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ShipEngine::UpdateCarrierSettingsRequestBody` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ShipEngine::UpdateCarrierSettingsRequestBody`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'include_barcode_with_order_number')
        self.include_barcode_with_order_number = attributes[:'include_barcode_with_order_number']
      end

      if attributes.key?(:'receive_email_on_manifest_processing')
    ...
    def hash
      [nickname, should_hide_account_number_on_archive_doc, is_primary_account, pickup_type, smart_post_hub, smart_post_endorsement, include_barcode_with_order_number, receive_email_on_manifest_processing, use_carbon_neutral_shipping_program, use_ground_freight_pricing, use_consolidation_services, use_order_number_on_mail_innovations_labels, mail_innovations_endorsement, mail_innovations_cost_center, use_negotiated_rates, account_postal_code, invoice, email].hash
    end
