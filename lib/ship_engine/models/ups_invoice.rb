  ...
  class UpsInvoice
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ShipEngine::UpsInvoice` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ShipEngine::UpsInvoice`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@invoice_amount.nil? && @invoice_amount < 0
        invalid_properties.push('invalid value for "invoice_amount", must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if !@invoice_amount.nil? && @invoice_amount < 0
      true
    end
    ...
    def invoice_amount=(invoice_amount)
      if !invoice_amount.nil? && invoice_amount < 0
        fail ArgumentError, 'invalid value for "invoice_amount", must be greater than or equal to 0.'
      end
