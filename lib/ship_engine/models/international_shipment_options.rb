  ...
  class InternationalShipmentOptions
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ShipEngine::InternationalShipmentOptions` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ShipEngine::InternationalShipmentOptions`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'non_delivery')
    ...
    def list_invalid_properties
      ...
      if @non_delivery.nil?
        invalid_properties.push('invalid value for "non_delivery", non_delivery cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @non_delivery.nil?
      true
    end
    ...
    def hash
      [contents, non_delivery, customs_items].hash
    end
