  ...
  class CarrierAdvancedOption
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ShipEngine::CarrierAdvancedOption` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ShipEngine::CarrierAdvancedOption`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@default_value.nil? && @default_value.to_s.length < 1
        invalid_properties.push('invalid value for "default_value", the character length must be great than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if !@default_value.nil? && @default_value.to_s.length < 1
    ...
    def default_value=(default_value)
      if !default_value.nil? && default_value.to_s.length < 1
        fail ArgumentError, 'invalid value for "default_value", the character length must be great than or equal to 1.'
      end
