  ...
  class BulkRate
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `ShipEngine::BulkRate` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `ShipEngine::BulkRate`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'rate_request_id')
    ...
    def list_invalid_properties
      ...
      if @rate_request_id.nil?
        invalid_properties.push('invalid value for "rate_request_id", rate_request_id cannot be nil.')
      end
    ...
    def valid?
      return false if @rate_request_id.nil?
